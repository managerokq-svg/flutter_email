...
class VMessageApiService {
  ...
  Future<Map<String, dynamic>> toggleReaction({
    ...
    late Response res;
    try {
      res = await _messageApi!.toggleReaction(
        roomId,
        messageId,
        {'emoji': emoji},
      );
  ...
  Future<List<VReactionDetail>> getMessageReactionsDetails({
    required String roomId,
    required String messageId,
  }) async {
    late Response res;
    try {
      res = await _messageApi!.getMessageReactionsDetails(roomId, messageId);
    } on SocketException {
      throw VUserInternetException(exception: "SocketException");
    } on TimeoutException {
      throw VUserInternetException(exception: "TimeoutException");
    ...
    throwIfNotSuccess(res);
    final data = extractDataFromResponseList(res);
    return data
        .map((e) => VReactionDetail.fromMap(e as Map<String, dynamic>))
        .toList();
  }
