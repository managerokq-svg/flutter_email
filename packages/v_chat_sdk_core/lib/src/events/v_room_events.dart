...
class VTotalUnReadMessagesCount extends VAppEvent {
  final int count; // Total unread count.

  const VTotalUnReadMessagesCount(this.count);
...
/// Event for the total unread count.
class VTotalUnReadRoomsCount extends VAppEvent {
  final int count; // Total unread count.

  const VTotalUnReadRoomsCount(this.count);
...
class VUpdateRoomMemberMention extends VRoomEvents {
  final int count;

  const VUpdateRoomMemberMention({
    required this.count,
    required super.roomId,
  });
...
class VUpdateRoomOneSeenEvent extends VRoomEvents {
  final bool isEnable; // The new mute status.

  const VUpdateRoomOneSeenEvent({
    required super.roomId,
    required this.isEnable,
  });
}

class VUpdateLocalRoomNickNameEvent extends VRoomEvents {
  final String name;

  const VUpdateLocalRoomNickNameEvent({
    required super.roomId,
    required this.name,
  });
...
// NEW: Event for updating room wallpaper.
class VUpdateRoomWallpaperEvent extends VRoomEvents {
  final String? wallpaperPath; // null indicates reset to default

  const VUpdateRoomWallpaperEvent({
    required super.roomId,
    required this.wallpaperPath,
  });

  @override
  List<Object?> get props => [super.props, wallpaperPath];
}
