...
import 'package:v_chat_sdk_core/src/native_api/local/native_local_sticker.dart';
...
class VLocalNativeApi {
  ...
  late final NativeLocalSticker sticker;
  ...
  Future<VLocalNativeApi> init() async {
    ...
    // On web, database is null - we still need to initialize with null database
    // The individual classes will handle null database internally
    message = NativeLocalMessage(database);
    if (database != null) {
      await message.prepareMessages();
    }
    room = NativeLocalRoom(database, message);
    sticker = NativeLocalSticker(database);
...
class VRemoteNativeApi {
  ...
  Future<http.Response> nativeHttp(
    ...
    mutableHeaders['authorization'] = "Bearer ${VAppPref.getHashedString(
      key: SStorageKeys.vAccessToken.name,
    )}";
    mutableHeaders["clint-version"] = VAppConstants.clintVersion;
    switch (method) {
      case VChatHttpMethods.get:
        return http.get(uri, headers: mutableHeaders);
      case VChatHttpMethods.post:
        return http.post(uri, body: body, headers: mutableHeaders);
      case VChatHttpMethods.patch:
        return http.patch(uri, body: body, headers: mutableHeaders);
      case VChatHttpMethods.delete:
        return http.delete(uri, body: body, headers: mutableHeaders);
      case VChatHttpMethods.put:
        return http.put(uri, body: body, headers: mutableHeaders);
    }
  }
